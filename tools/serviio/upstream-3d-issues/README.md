...
|---|---|---|---|---|
| 1 | `ffmpeg-eexist-bug.md` + `ffmpeg-eexist-facts.md` | FFmpeg | code.ffmpeg.org tracker (bug) | **POSTED 2026-09-16** — [issue #24530](https://code.ffmpeg.org/FFmpeg/FFmpeg/issues/24530) (owner's own reworded text + reproducer attached); **fix PR [#24643](https://code.ffmpeg.org/FFmpeg/FFmpeg/pulls/24643) opened 2026-09-23** |
...
| 8 | *(Opus session)* mpv issue + PR — drafts and patch archived at `/K3D/GitHub/EchoSystems_Stereo3D/` | mpv (player side) | github.com/mpv-player/mpv | **FILED 2026-09-16, IN REVIEW** — [#18489](https://github.com/mpv-player/mpv/issues/18489) (stream-signalled stereo 3D ignored) + [PR #18490](https://github.com/mpv-player/mpv/pull/18490) (2 commits, Fixes #18489); review round 1 answered 2026-09-16 — 8 threads, owner's own words, 3 follow-ups offered (see below). **ROUND 2, 2026-09-21/22**: kasper93 CHANGES_REQUESTED with four precise asks, plus a separate comment on issue #17632 about our clips (full/half from aspect is a guess; Matroska RFC 9559 §18.10 is where per-eye geometry should live but files don't follow it). v2 rework landed his asks exactly: mp_image mapping with QUINCUNX and fixed comments, wrapper reduced to the guarded container fallback, no mpv-side persistence (moved to a planned FFmpeg `ff_h2645_sei_to_frame()` lane), no vd_lavc fallback in v2; force-pushed as `049bb33bba`+`fc2ac9e8cb` (author Capitain_Jack@yahoo.com, the LLM-assistance paragraph kept per mpv's own rule). Runtime-measured before push: SEI-only MP4 flips on IDR cadence (the documented stock-FFmpeg gap), container-tagged MKV unchanged. v1 never injected the SEI per frame — x264 repeats it per IDR (fixtures: frames 1/49/97/145/193 at GOP 48); per-frame broadcast carriage + cancel-flag 2D/3D transition assistance are ETSI TS 101 547-2/DVB features, file content keeps a constant arrangement. **ROUND 2 ANSWERED 2026-09-21/22**: seven line replies posted (ids 4067137433–4067138147), summary comment [5768736315](https://github.com/mpv-player/mpv/pull/18490#issuecomment-5768736315) (four asks, honest IDR-cadence behavior note, MKVToolNix precedent, closures restated: closes #18489, unblocks #17632/#18348), and the norm-cited reply to his clips comment posted on #17632 as [comment 5768747371](https://github.com/mpv-player/mpv/issues/17632#issuecomment-5768747371). Re-request review is the owner's browser click (GitHub REST refuses reviewer requests from non-collaborators, 404); resolve buttons stay with the reviewer |
