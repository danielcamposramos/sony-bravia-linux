...
| 11 | `kodi-issue.md` | Kodi (player side, 21k★) | github.com/xbmc/xbmc (bug_report template) | **POSTED 2026-09-18** — [issue #29337](https://github.com/xbmc/xbmc/issues/29337). Source-read on master 6c678081054b: stereo mode comes only from container metadata (`GetStereoModeFromMetadata`: Matroska `stereo_mode`, WMV `Stereoscopic`) and filename tokens (`StereoscopicsManager`); no `AV_FRAME_DATA_STEREO3D` / `AV_PKT_DATA_STEREO3D` / `frame_packing` anywhere in the tree, and no prior issue on the tracker. Checkable reproducer: an x264 `frame-packing=3` MP4 with the SEI and no container tag. Fix direction: frame side data for software decode, parse SEI 45 for hardware decode, container tag and filename stay authoritative. Same gap as mpv #18489. No debuglog (Kodi not installed here; said so in the issue rather than faking one). Email-trigger watch, never poll |
