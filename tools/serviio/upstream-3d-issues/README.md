...
| 10 | `jackdesbwa-message.md` | JackDesBwa (PhereoRoll3D / PhotoRoll3D — the stereo-photo side) | GitHub issue on PhereoRoll3D or lower-noise channel; see the channel note in the draft | **DRAFT, HELD (2026-09-18)** — not an issue report but the act-three connect: our video-side SEI campaign as the counterpart of his client-side photo display modes, the 3D-photo gallery lane on the 2011 BRAVIA (his MIT client = the API map + Dubois reference), and two community questions only he can answer (practical anaglyph→SBS extraction; which stereo-photo sources still worth an adapter — his PhotoRoll3D WIP names the multi-source idea but ships no adapters yet, verified 2026-09-18). Per act-three doctrine ideally posted after the first measurements so it links a working thing; owner rewords and posts |
