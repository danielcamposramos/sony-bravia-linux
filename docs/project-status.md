...
| Phereo 3D-photo gallery lane | **CHARTERED 2026-09-18** — the 3D photo player these sets never got (Sony's stock slideshow was 2D-only on a 3D panel). Two sources, one server-rendered gallery: Serviio `imageItem`s (own library, existing browse pattern) + the phereo open API (no auth, JSON, endpoints mapped from the MIT client [PhereoRoll3D](https://github.com/JackDesBwa/PhereoRoll3D) — which also carries the anaglyph-Dubois reference for our colorimetry chain; [PhotoRoll3D](https://github.com/JackDesBwa/PhotoRoll3D) is the author's multi-source WIP). Front A pure: SBS delivered, glasses do the 3D, zero pixel conversion. adultFilter on by default. Measured at charter: host up, static assets 200, API path 504 (backend struggling) — build defensively, cache, re-verify from the LAN. **Era-Opera check done:** the Qt/QML clients can't run on the TV (compiled apps), but our server-rendered reimplementation needs only measured capabilities (images, link pagination, the inline JS the lanes already use); JSON/Dubois/shader work stays server-side. PhotoRoll3D verified: "more online sources" is stated goal, not shipped code — one commit, QML shell + shader renderer, no adapters yet. Open test inherited: does the *native* DLNA/USB photo path allow the manual 3D switch for an SBS photo? | `legacy-3d-formats.md` |
| 3D catalog + categories | **CHARTERED 2026-09-18 (prepared, held for owner's go — nothing built)** — "both ends": the serving side must *know* the contents are 3D. A scanner ("3D cataloger") walks the library and emits a 3D index (JSON, keyed to Serviio's paths): item → type → evidence → confidence → owner override. Flags: `sbs`/`jps`/`mpo`/`tab`/`row`/`col`/`anaglyph`, photos and video alike (video detection reuses act-one SEI/tag tooling). Portal gains an entire top-level **3D category** — *All 3D photos*, *All 3D movies/series/videos*, browsable by type. Honest scope: jps/mpo/SEI certain from the file; interleaved/anaglyph detection heuristic, owner override always wins. Serviio unchanged (index enriches browse at the portal layer); native 3D metadata in servers = act-two doctrine, after the demonstration. Also: anaglyph runs **both directions** now — SBS→anaglyph display (Dubois reference in PhereoRoll3D) and the inverse anaglyph→SBS (monochrome extraction well-posed; re-color heuristic; disparity reconstruction research-grade; phereo's own anaglyphs are derived renditions — fetch the SBS original). Draft outreach to JackDesBwa prepared (the video-side connect + two community questions), held unposted | `legacy-3d-formats.md`, `upstream-3d-issues/jackdesbwa-message.md` |
