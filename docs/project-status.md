...
| HDMI/KMS stereo output | **SBS-half and full frame packing now proven on real hardware.** Patched amdgpu on its own HDMI output: 22/0 → 51/29 modes and SBS-half 1080p60 auto-engages the KDL-46HX855 with visible depth. Stock nouveau on the GA106's separate HDMI cable/input: the same mode exposure plus SBS-half 1080p60 and full 1080p24 frame packing both auto-engage; FP uses the DRM-correct 1920x2205 scanout and restored the proprietary NVIDIA desktop/CUDA stack cleanly. Source audit shows the remaining AMD FP boundary: amdgpu's DRM glue leaves `timing_3d_format=NONE` although DC contains frame-packing timing, view, OPP and packet machinery. Identical AMD FP hardware run is next. Dual-surface composition is independently proven in wiz3D's left/right-backbuffer SBS/TaB output code | `tools/stereo-modeset/`, `docs/dual-surface-hdmi-3d.md` |
