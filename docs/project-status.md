...
| NVIDIA/nouveau HDR lane | **SOURCE SHAPE CONFIRMED; OUTPUT NOT YET CLAIMED.** The proprietary stack's open glue already exposes `HDR_OUTPUT_METADATA`/Colorspace and hands metadata to closed NVKMS, so #1384 correctly leaves its link-depth/output-policy repair with NVIDIA. In nouveau 7.3-rc4, the corresponding KMS properties and CTA-861.3 DRM InfoFrame path are absent. The audit found the exact HDR boundary: NVIF/nvkm carry only 17-byte AVI/VSI packets, while HDR needs 30 bytes. It also found two credible open routes—Valve's new GB20x 36-byte generic InfoFrame writer and r535's 36-byte `SET_OD_PACKET` GSP command—but neither proves the GA106 slot contract yet. **Deep-colour prerequisite now mapped separately:** nouveau ignores HDMI VSDB `DC_30/DC_36` for link selection, exposes no `max bpc`, and leaves TMDS SOR depth at default even though its display classes define 36-bpp RGB. The owned HX855 makes 12-bpc SDR directly testable at 1080p60: 148.5 × 1.5 = 222.75 MHz against the EDID's tight 225 MHz ceiling. A reversible module-swap/OSD experiment is designed; this would prove transport, not HDR. **Run 2 measured 2026-09-22:** the experimental patch exposed `max bpc` (8–12) on HDMI-A-2, accepted `max bpc=12`, and committed the 1080p60 36-bpp modeset with zero driver-side errors; the sink refused the wire with its "incompatible signal" OSD — engagement end to end is proven working, and the measured missing half is the General Control Packet deep-colour indication that the plan doc flagged as GA106's open question. Owner aborted the hold deliberately (~50 s in) and rebooted; kernel/desktop restore clean, docker restore skipped because a SIGTERM-killed `sh` never runs its EXIT trap — gap recorded as a harness defect, containers restarted manually. Native HDR still requires an HDR sink or analyzer. The cross-vendor high-precision SDR and native-HDR map now lives in [`awesome-linux-hdr`](https://github.com/danielcamposramos/awesome-linux-hdr). | `docs/research/nouveau-hdr-gap-2026-09-22.md`, `docs/research/nouveau-hdmi-deep-colour-plan-2026-09-22.md`, `docs/upstream/issue-tracker.md` |
