...
**NVIDIA-proprietary lane — runs 12-16, 2026-09-21**: 615.71.09 (no swap, no removal) prunes every stereo mode — baseline probe 17 modes / 0 stereo forever (`stereo_allowed` appears nowhere in the open glue). BUT the glue exposes an `NV_HDMI_VSIF_METADATA` connector blob property whose payload crosses into NVKMS with the modeset (nvidia-drm-connector.c:592-600 → NVKMS), so `stereo-modeset ... isolate vsif` does a plain 2D modeset plus a userspace-injected 6-byte HDMI 1.4 3D VSIF. Matrix on the KDL-46HX855: **SBS-half 1080p60 PASS twice** (runs 13/14, auto-engage + picture); TaB engages everywhere but fails per timing — 1080p60 = whole stream muted incl. the 2D-toggle path (runs 14/15), 720p60 = video decodes under the 2D toggle while 3D stays lit-black (run 16); frame packing refused by design (doubled link timing needs real mode support). Same sink ate identical TaB bytes from the kernel-driven AMD link (run 11), so the boundary is NVIDIA-delivery-specific, strengthening the ask. Endstate: GitHub issue to NVIDIA/open-gpu-kernel-modules carrying the pipe evidence + matrix + the minimal glue delta (expose `stereo_allowed`); **Daniel posts it himself**. Records: `tools/stereo-modeset/run12…run16-*.log`, harness `run-nvidia-test.sh`
