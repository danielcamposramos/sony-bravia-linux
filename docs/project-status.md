...
| NVIDIA/nouveau HDR lane | **SOURCE SHAPE CONFIRMED; OUTPUT NOT YET CLAIMED.** The proprietary stack's open glue already exposes `HDR_OUTPUT_METADATA`/Colorspace and hands metadata to closed NVKMS, so #1384 correctly leaves its link-depth/output-policy repair with NVIDIA. In nouveau 7.3-rc4, the corresponding KMS properties and CTA-861.3 DRM InfoFrame path are absent. The audit found the exact boundary: NVIF/nvkm carry only 17-byte AVI/VSI packets, while HDR needs 30 bytes. It also found two credible open routes—Valve's new GB20x 36-byte generic InfoFrame writer and r535's 36-byte `SET_OD_PACKET` GSP command—but neither proves the GA106 slot contract yet. Local work can cover build, properties, atomic state, packed bytes, and SDR/3D regressions; final HDR-on-wire acceptance requires an HDR sink or analyzer, which this bench does not have. | `docs/research/nouveau-hdr-gap-2026-09-22.md`, `docs/upstream/issue-tracker.md` |
