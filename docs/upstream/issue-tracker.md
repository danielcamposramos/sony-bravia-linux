...
|---|---|---|---|---|---|---|
| 1 | NVIDIA HDMI 1.4 3D: `stereo_allowed` glue delta | github.com/NVIDIA/open-gpu-kernel-modules (issues) | issue # TBD at post time | **DRAFT APPROVED** — Daniel's reviewed edit on disk (`docs/upstream/nvidia-issue-draft-hdmi-3d.md`); contribution patch attaches; CLA engages only if NVIDIA invites a PR | **Daniel posts** → then NVIDIA | one gentle `gh api repos/NVIDIA/open-gpu-kernel-modules/issues/<n>` check on request; no crawling |
